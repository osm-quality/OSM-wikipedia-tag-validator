...
import nominatim_cached_on_disk
import sqlite3
import json
import config
import database
...
def check_with_nominatim_is_within_given_country(root_osm_object_url, lat, lon, target_country, debug):
    ...
    instance = nominatim_cached_on_disk.get.Nominatim(cache_folder="/media/mateusz/OSM_cache/OSM_cache/nominatim", user_agent="Wikipedia validator")
    if instance.get_nominatim_country_code(lat=lat, lon=lon) == target_country:
        return True
    else:
        if debug:
