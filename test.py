...
def process_element(element, forced_refresh=False):
    ...
    if link == "":
        ...
        language_code = get_language_code_from_link(link)
        article_name = get_article_name_from_link(link)
        if args.expected_language is not None and args.expected_language != language_code:
            output_element(element, "missing " + args.expected_language + ":")
            return True
...
def parsed_args():
    ...
    parser.add_argument('-expected_language', '-l', dest='expected_language', type=str, help='expected language code')
